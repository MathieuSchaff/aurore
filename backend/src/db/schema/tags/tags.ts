...
export const productTagLinks = pgTable(
  'product_tag_links',
  {
    ...
    relevance: relevanceEnum('relevance').notNull().default('secondary'),
    // Auto-tag intake deletes only rows where source != 'manual' before inserting again,
    // so manual curation survives retag cycles. Default 'manual' keeps the rows that
