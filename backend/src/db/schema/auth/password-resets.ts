...
// Mirrors email_verifications: lookup table read before identity is known (forgot
// and reset happen before any session), so it stays outside RLS like its sibling.
// Only the sha256 token hash is stored; the raw token lives only in the reset link.
