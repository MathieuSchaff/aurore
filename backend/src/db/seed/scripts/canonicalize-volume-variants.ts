 ...
 *
 * Usage:
 *   bun run backend/src/db/seed/scripts/canonicalize-volume-variants.ts --dry
 *   bun run backend/src/db/seed/scripts/canonicalize-volume-variants.ts
 *
...
if (!existsSync(AUDIT_PATH)) {
  ...
  console.error('Run: bun run backend/src/db/seed/scripts/audit-imported-products.ts --write')
  process.exit(1)
}
