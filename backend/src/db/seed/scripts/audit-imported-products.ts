 ...
 *
 * Usage:
 *   bun run backend/src/db/seed/scripts/audit-imported-products.ts
 *   bun run backend/src/db/seed/scripts/audit-imported-products.ts --write
 */
...
function classifyPair(a: UnifiedProductSeed, b: UnifiedProductSeed): ClassifyResult {
  ...
  const conflict = flags.some(
    (f) => f.startsWith('num-diff:') || f.startsWith('tint-diff:') || f.startsWith('gamme-letter:')
  )
