...
### 8.4-8.5 ~~Tags dental + `productTagData` skincare-only~~ — résolus
...
Déplacé dans [`STATE-GLOSSARY.md`](./STATE-GLOSSARY.md). Lecture requise quand on touche un schéma shared (`type`, `category`, `kind`, `tagType`).

---

## 11. Pipeline images

Déplacé dans [`IMAGES.md`](./IMAGES.md). Lecture requise quand on touche CDN / mapping / upload S3.
