...
**✅ Done (23) :** R1, R2, R3, C1, C2, C3, C4, C5, X1, C6, S1, S2, S3, S4, T1, **I1** (sweep 11 slugs commit `16b8ec60` + cleanup mutex stale + interaction-driven peau-sensible avoid `0c07e9a8`) + **X2** + **Tier 2** (eczema-atopie / effet-protecteur / repulpant) + **D.3 hoist** (2026-05-09) + **T1 sans-X family** (2026-05-09) + **R4 semi-occlusif** (2026-05-09).
...
| 🟡 S5 | texture-gel/mousse/stick | M | **Bloqué** — champ `products.texture` requis (T3) |
...
| Ordre | Item | Effort | Pourquoi |
|------:|------|--------|----------|
| 7 | **X3** ProductInteraction[] mapping | M | Exploite lib algo-derm sous-utilisée. Ouvre warnings interaction. |
...
| 14 | **T4** brand-level vegan/cruelty/natural | L | Données externes (brand DB / labels certifiés), pas dérivable INCI. |
...
| 16 | **S5** texture-gel / mousse / stick | M | T3 (champ texture) |
...
| X3 | **Surface algo-derm `ProductInteraction[]`** dans le tagging — actuellement seules les interactions grossesse sont consommées. Ajouter mapping interaction → tag/warning. | M | Sous-utilisation de la lib. |
...
| T3 | **Champ `texture` produit** distinct de `kind` | L | Schéma : ajouter `products.texture` (`gel` / `creme` / `mousse` / `stick` / `huile` / `lait` / `eau` / `baume` / `patch`) ; alimente sensoriel S5 et raffine TYPE_/TEXTURE_. |
| T4 | **Champ `is_natural` / `is_vegan` / `is_cruelty_free`** au niveau marque ou produit | L | Pas dérivable d'INCI seul. Source = brand DB / label certifié. |
