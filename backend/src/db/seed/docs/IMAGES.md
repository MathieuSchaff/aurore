# Images produits — Pipeline & CDN

> **À propos :** Où vivent les images produit, comment elles sont mappées aux seeds, et le pipeline upload S3 → CDN Bunny → patch `imageUrl`. Répond à « où sont les images ? », « comment ça arrive en prod ? ».
>
> **Audience** : humain + IA qui reprend le chantier.
> **Important** : `output/` est gitignoré ; les images locales ne sont pas versionnées.
...
- Pipeline import scrap amont : [`IMPORT_PIPELINE.md`](./IMPORT_PIPELINE.md) (archivé)
- Format seed produit : [`SEED_FORMAT.md`](./SEED_FORMAT.md) §3.4 (`imageUrl`)
- État global seed : [`STATE.md`](./STATE.md) §3.3 (`UnifiedProductSeed.imageUrl`)
