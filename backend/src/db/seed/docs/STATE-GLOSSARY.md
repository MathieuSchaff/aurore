# Glossaire seed — sémantique des champs

> Glossaire cross-entité : les mots `type`, `category`, `kind`, `tagType` ont des sens différents selon la table. Ce doc pose qui a quoi.

> **Architecture** : voir [`STATE.md`](./STATE.md). **Référencé depuis CLAUDE.md** pour la sémantique des champs seed.

---

## 1. Vue d'ensemble — un tableau, trois entités
...
### 2.3 Ce qui n'existe pas pour un ingrédient
...
### 3.1 Axe `category` — domaine métier
...
### 4.2 Axe `tagType` — famille de tag
...
### 4.3 Axe `relevance` — lien entité↔tag
...
## 5. Croisements domaine ingrédient × domaine produit
...
## 6. État de la validation — matrice
...
## 7. Cheat sheet — quel champ regarder ?
