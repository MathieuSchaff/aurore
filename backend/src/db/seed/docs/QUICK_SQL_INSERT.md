...
- **`image_url`** : convention finale = `https://aurore-cdn.b-cdn.net/products/<slug>.webp` (Bunny CDN). Si pas encore uploadé, on peut stocker l'URL d'origine et migrer plus tard via les scripts de `backend/src/db/seed/scripts/upload-images.ts`. Voir `STATE.md §11`.
