...
function verifyIngredientProductSlugs() {
  ...
  entries.forEach(([productSlug, ingredients]) => {
    ...
    if (productSlug === 'undefined') {
      console.error(`❌ ERREUR : Une clé dans ALL_PRODUCT_INGREDIENTS_MAP est 'undefined'. Cela signifie probablement qu'un slug est manquant dans allProductSlugs.`)
      errorCount++
      return
    }

    if (!productSlugsValues.has(productSlug)) {
      console.error(`❌ ERREUR : Le slug produit '${productSlug}' n'est pas défini dans ALL_PRODUCT_SLUGS.`)
      errorCount++
    }

    if (!dataProductSlugs.has(productSlug)) {
      console.warn(`⚠️  ATTENTION : Le slug produit '${productSlug}' est présent dans les ingrédients mais absent de allProductData.`)
    ...
    if (!Array.isArray(ingredients)) {
      console.error(`❌ ERREUR : Les ingrédients pour le produit '${productSlug}' ne sont pas un tableau.`)
      errorCount++
      return
    }
    ...
    ingredients.forEach((ing: any, index: number) => {
      if (!ing.slug) {
        console.error(`❌ ERREUR : L'ingrédient à l'index ${index} pour le produit '${productSlug}' n'a pas de slug.`)
        errorCount++
      } else if (!ingredientSlugsValues.has(ing.slug)) {
        console.error(`❌ ERREUR : Le slug d'ingrédient '${ing.slug}' pour le produit '${productSlug}' est inconnu dans INGREDIENT_SLUGS.`)
        errorCount++
      }
    })
  })
  ...
  if (errorCount === 0) {
    console.log(`✅ TOUT EST CORRECT ! Tous les ingrédients ont un product slug valide et leurs propres slugs sont corrects.`)
  } else {
    console.error(`🛑 ${errorCount} ERREUR(S) DÉTECTÉE(S). Veuillez corriger les problèmes ci-dessus.`)
  }
}
