// Registry for new-format brand seeds.
// To add a brand: import its seed array, spread it into `allUnified`.
...
export const unifiedProductData = allUnified.map(
  ({ tags: _tags, keyIngredients: _ki, ...product }) => product,
)

export const unifiedProductTagsMap: Record<string, ProductTagGroups> = Object.fromEntries(
  allUnified.map(p => [p.slug, p.tags]),
)

export const unifiedIngredientsMap: Record<string, Ingredient[]> = Object.fromEntries(
  allUnified
