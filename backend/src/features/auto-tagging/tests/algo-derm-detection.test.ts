...
describe('algo-derm-detection', () => {
  ...
  test('emits non-irritant for recognized gentle INCI', () => {
  ...
  test('omits non-irritant for leave-on with SLS and fragrance', () => {
  ...
  ])('voids the non-irritant claim for %s in an otherwise gentle formula', (_label, inci) => {
    const slugs = new Set(detectAutoTags(inci, 'moisturizer').map((t) => t.slug))
    expect(slugs.has(S.NON_IRRITANT)).toBe(false)
  })

  test('requires at least 0.60 coverage for non-comedogene', () => {
  ...
  test('bypasses coverage and confidence floors when disableFloors is set', () => {
  ...
  test('emits absence tags for silicones, oils and allergens in clean leave-on INCI', () => {
  ...
  test('keeps sans-sulfates for cleansers and drops it for leave-on products', () => {
  ...
  ] as const)('suppresses %s when its offending ingredient is present', (slug, inci, kind) => {
