...
// The name check misses acid-named products (e.g. "Chestnut AHA Essence"), wrongly dropping
...
// It reads the dose, so we trust it over the name check when it is confident:
// a dose under c50 is a pH adjuster even when the name says exfoliant
// Near the knee of the curve it stops being confident, so we fall back to the name check
// and the %-rescue
...
// In v38 the pin returned 0, so the product went to the name check and stayed in the gold set
// as a false positive
...
// cannot vouch for the product, so legacy keep. No lookups: identical to the bare name check
