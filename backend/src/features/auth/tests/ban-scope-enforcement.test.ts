...
describe('Per-scope ban enforcement (requireNotBannedScope)', () => {
  ...
  it('global ban blocks scope-gated writes too (via requireNotBanned upstream)', async () => {
    ...
    const body = await expectError<{ scope?: string }>(res, HTTP_STATUS.FORBIDDEN, 'banned')
    // upstream gate fires first, so details.scope is absent (only set by scope middleware)
  ...
  // Ban gates publication only, not private notes.
  it('review_publish ban does NOT block PUT review with isPublic:false (private notes)', async () => {
    ...
    // No user_product seeded under ANY_UUID, so the handler reaches the service and
