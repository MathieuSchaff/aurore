 ...
 * production: APP_DATABASE_URL connects as `app_runtime` (no BYPASSRLS). role_requests
 * carries tenantPolicies only: tenant_isolation + admin_bypass. There is deliberately
 * no contributor policy: granting a role is account elevation, admin-only. This file
 * proves owner-only read/write, the admin bypass, and the contributor lockout under prod RLS.
 */
...
describe('role_requests RLS under app_runtime', () => {
  ...
  it('owner can cancel own row; a non-owner user cannot; admin can', async () => {
    ...
    // so each branch gets its own owner instead of seeding the same one twice.
    ...
    // An UPDATE from a user who isn't the owner matches 0 rows (tenant_isolation hides the row).
