...
describe('Skincare Seeding Transformation', () => {
  ...
  it('should extract capacity correctly', () => {
    const { name, totalAmount, unit } = extractCapacity("SK-II Brightening Derm Revival Masks, 10 Count", "SK-II")
  ...
  it('should extract capacity from complex strings', () => {
    const { name, totalAmount, unit } = extractCapacity("IsNtree Aloe Fresh Type Soothing Gel, 10.14 fl oz/300 mL", "IsNtree")
...
describe('Database Seeding Integration', () => {
  it('should seed a product and its ingredients into the test database', async () => {
    ...
    for (const ingName of ingNames) {
      ...
      let ingredient;
      try {
        ingredient = await createIngredient(user.id, {
          name: ingName,
          slug: ingSlug,
          description: '',
          content: '',
    ...
    const linkedIngredients = await testDb.select().from(productIngredients).where(eq(productIngredients.productId, product.id))
