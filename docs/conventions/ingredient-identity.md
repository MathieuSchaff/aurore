...
- **Run it again after a fresh seed or an algo-derm bump:**
  `just catalog-backfill-canonical-key --write` then `just db-snapshot`.
- `just db-seed` already runs the backfill again (seed inserts leave the column NULL).
