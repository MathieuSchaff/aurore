...
Four code paths decide what happens when an emitted auto-tag pair meets an existing `product_tag_links` row: intake (`auto-tagging/write.ts`, SQL DELETE-nonmanual + INSERT `onConflictDoNothing`), backfill (`runners/backfill/classify.ts`, pure additive-only), the reconcile dry-run (`runners/backfill/reconcile-diff.ts`, pure full-sync prediction), and the seed (`db/seed/seeders/merge-product-tag-pairs.ts`, pure stream merge). The obvious consolidation is one pure "tag reconciliation policy" module (a function of `want`, `stored` and `manual` returning actions, modes `additive | full-sync`). We reject that module and instead lock the one real gap with a cross-pinning test: `tests/reconcile-parity.test.ts` proves `diffReconcileProduct` predicts exactly what `writeTagsForProduct` persists.

## Why

- **The four paths are two policies, each in the encoding that fits it.** Additive (backfill) and full-sync (intake; reconcile WRITE applies the intake primitive) are genuinely different behaviours, not copies. The seed has no `stored` side at all (fresh DB): its merge is a different decision and lives in its own module.
...
- **The manual-safety invariant is already singular where it matters.** It is a named domain concept, the manual tag, and each encoding is pinned by its own test (`backfill-classify.test.ts`, `auto-tag-manual-overlap.test.ts`, `merge-product-tag-pairs.test.ts`, and now the parity test).
...
- Reopen only if a **third real policy** appears (not a new caller of an existing one), or if drift between dry-run and apply ships despite the parity test.
