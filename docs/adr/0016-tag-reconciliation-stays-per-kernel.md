...
- Future architecture reviews should not propose the shared policy module again while the premises hold. Reopen if a **third real policy** appears (not a new caller of an existing one), or if dry-run↔apply drift ships despite the parity test.
