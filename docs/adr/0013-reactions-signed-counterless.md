...
# Reactions are signed and counterless: a list of who, never a tally

Aurore ships lightweight reactions (`merci` / `moi-aussi` / `soutien`) on Posts, Threads, and their replies, as an entraide signal. This ADR fixes the doctrine: a reaction is always **signed** (attributable to its author, never anonymous), the API exposes only the **list of reactors per kind** (pseudonyms) and **never a count**, and **no surface anywhere orders or gates visibility by reactions**. Reviews are deliberately never reactable. Direct lineage of [ADR-0005](0005-public-reviews-opt-in-attributable-ratings.md)'s antileaderboard tripwire.

## Why

The obvious shape for "react to a post" is a like button with a number: the industry default. That default is exactly what the antipattern guide refuses: an aggregate tally turns a warm acknowledgement into a popularity score, and a score invites sort-by-popularity, which invites a leaderboard. ADR-0005 carved a bounded exception for *self-authored* 1-5 ratings precisely because they are signed and never Aurore-aggregated; reactions stay on the same side of that line by construction.
...
An anonymous reaction is a vote: it measures volume, not people. A signed one is a person saying *I'm here* to another person ("le vivant : réagir sans liker-voter").

## Considered options

- **A. Like button with a count (+ optional sort-by-popularity).** Rejected: the count is an Aurore-computed aggregate, and it structurally invites ranking: the exact leaderboard ADR-0005 stays behind.
...
- `reactable_type` deliberately excludes `review`; the service rejects it. A Review is a deposit leaf, not a conversation surface (the antipattern guide's "feuille-dépôt").
...
- Several kinds at once are allowed: the UNIQUE key includes `kind`, so one user may hold `merci` and `soutien` on the same target at once. `POST` ensures a kind on (idempotent insert), `DELETE` ensures it off; the client picks the verb from the viewer's current kinds.
- The read API returns `{ reactions: { <kind>: [{ username, profilePublic }] }, viewerKinds }`. `viewerKinds` drives button pressed-state **without a count**. No endpoint, feed, or list orders by reactions (the social feed orders by recency or similarity only).
...
- Creating a reaction is gated by `requireJwtAuth` + the global-ban floor only, no dedicated `ban_scope`. Reactions are low-stakes, signed, and bounded by the UNIQUE key (it cannot be flooded); a dedicated scope is deferred until abuse is real.
