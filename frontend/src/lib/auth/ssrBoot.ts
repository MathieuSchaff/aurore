...
import { type ProductDomainTab, type SsrBootResponse, ssrBootResponseSchema } from '@aurore/shared'

import { createServerOnlyFn } from '@tanstack/react-start'
import { getRequestHeader } from '@tanstack/react-start/server'
import { hc } from 'hono/client'

export type SsrBootRequestQuery =
  | { view?: undefined }
  | ({ view: 'products'; category: ProductDomainTab } & Record<
      string,
      string | string[] | undefined
    >)
  | { view: 'product-detail'; slug: string }
...
export const loadSsrBoot = createServerOnlyFn(
  async (query: SsrBootRequestQuery = {}): Promise<SsrBootResult> => {
    ...
    if (hasRefreshCookie) {
      ...
      try {
        ...
        const response = await client.api.boot.$get(
          { query },
          {
