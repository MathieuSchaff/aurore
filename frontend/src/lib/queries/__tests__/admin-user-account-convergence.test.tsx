  ...
  it('refetches every mounted public identity surface after forcing a profile private', async () => {
    ...
    const { result } = renderHookWithProviders(
      () => ({
        ...
        similar: useQuery(socialQueries.similar('viewer')),
        search: useQuery(socialQueries.searchByConcern('anti-acne', 'viewer')),
        feed: useQuery(socialQueries.feed({ tone: 'principal', order: 'recency' }, 'viewer')),
